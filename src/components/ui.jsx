...
export function Btn({ children, onClick, variant = 'gold', className = '', disabled }) {
  const styles = {
    gold: 'bg-gold text-navy-deep active:bg-gold-dim',
    ghost: 'bg-navy-card text-white border border-navy-line active:bg-navy-line',
    danger: 'bg-red-500/90 text-white active:bg-red-600',
    outline: 'bg-transparent text-gold border-2 border-gold active:bg-gold/10',
  };
  return (
    <button
      disabled={disabled}
      ...
      className={`font-display rounded-2xl px-5 py-4 text-lg tracking-wide select-none
        transition-transform active:scale-[0.97] disabled:opacity-40 ${styles[variant]} ${className}`}
    >
      {children}
    </button>
  );
